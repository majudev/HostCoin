...
#if defined(NDEBUG)
# error "Hostcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hostcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hostcoin Signed Message:\n";
...
// Hostcoin: eHRC at 3rd hard fork
...
void ThreadScriptCheck() {
    RenameThread("hostcoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Hostcoin
...
{
    // Hostcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
std::string CDiskBlockIndex::ToString() const
{
    ...
    str += strprintf("\n                hashBlock=%s, hashPrev=%s, hashParentBlock=%s)",
        ...
        //hostcoin unuse (auxpow.get() != NULL) ? auxpow->GetParentBlockHash().ToString().substr(0,20).c_str() : "-"
        "0");
    return str;
}
