    ...
    tab(tab)
{
    ...
    {
    case SendingTab:
        ui->labelExplanation->setText(tr("These are your Hostcoin addresses for sending payments. Always check the amount and the receiving address before sending coins."));
        ui->deleteAddress->setVisible(true);
        ui->signMessage->setVisible(false);
        break;
    case ReceivingTab:
        ui->labelExplanation->setText(tr("These are your Hostcoin addresses for receiving payments. It is recommended to use a new receiving address for each transaction."));
...
void AddressBookPage::on_copySecKey_clicked()
{
    ...
    if(!selection.isEmpty())
    {
        ...
        //HostcoinPrivateKey(private_key)
        CSecret vchSecret;
        ...
        //HostcoinPrivateKey(private_key).public_key()
        CPubKey vchPubKey;
        if ( !pwalletMain->GetPubKey(keyID, vchPubKey))
        {
            QMessageBox::warning(this, windowTitle(),
        ...
        //HostcoinPrivateKey(private_key).public_key().hash160()
